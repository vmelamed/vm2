...
public class DataTransform(TransformOptions? options = default)
{
    ...
    static Dictionary<Type, ConstantTransform> _constantTransforms = new()
    {
        ...
        { typeof(Half),     (n, x) => x.Add(new XElement(XmlElement.Half,          XmlConvert.ToString((float)n.Value!))) },
    ...
    internal ConstantTransform GetTransform(Type type)
    {
        ...
        XElement parent)
    {
        ...
        for (var i = 0; i < props.Length; i++)
        {
            ...
            {
                var transform = GetTransform(props[i].PropertyType);
