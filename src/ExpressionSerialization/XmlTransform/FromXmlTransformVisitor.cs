...
public partial class FromXmlTransformVisitor(Options options)
{
    ...
    static readonly Dictionary<XName, Func<FromXmlTransformVisitor, XElement, Expression>> _transforms = new()
    {
        ...
        // assignments
            ...
            { ElementNames.Default,              (v, e) => VisitDefault(e)               },
    ...
    public Expression Visit(XElement element) => _transforms[element.Name.LocalName](this, element);
    ...
    {
        var constantElement = e.Elements().FirstOrDefault();

        if (constantElement is null)
            throw new SerializationException("Expected constant element's contents.");

        return FromXmlDataTransform.ConstantTransform(constantElement);
    }
    ...
    static Expression VisitDefault(XElement e) => throw new NotImplementedException();
