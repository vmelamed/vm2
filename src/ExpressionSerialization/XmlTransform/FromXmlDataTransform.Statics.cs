...
partial class FromXmlDataTransform
{
    ...
    static readonly Dictionary<Type, Func<Type, Type, int, IEnumerable, object?>> _sequenceBuilders_ = new ()
    {
        ...
        [typeof(ArraySegment<>)]        = (gt, et, len, seq) => BuildWithConstructor(gt, et, TransformToArray(et, len, seq)),
        [typeof(Memory<>)]              = (gt, et, len, seq) => BuildWithConstructor(gt, et, TransformToArray(et, len, seq)),
        [typeof(ReadOnlyMemory<>)]      = (gt, et, len, seq) => BuildWithConstructor(gt, et, TransformToArray(et, len, seq)),
        [typeof(ConcurrentBag<>)]       = (gt, et, len, seq) => BuildConcurrentBag(gt, et, seq),
        [typeof(ConcurrentQueue<>)]     = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq),
        [typeof(ConcurrentStack<>)]     = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq.Cast<object?>().Reverse()),
        ...
        [typeof(LinkedList<>)]          = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq),
        [typeof(Queue<>)]               = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq.Cast<object?>()),
        [typeof(SortedSet<>)]           = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq),
        [typeof(Stack<>)]               = (gt, et, len, seq) => BuildWithConstructor(gt, et, seq.Cast<object?>().Reverse()),
